...
int move(int t, point snake[5][100], int len[5], int direct[5], GamePanel& gp, int& iterationtime)
{
	...
	/****************************************************************/
	int nextdirection = machine_move25(snake, len, direct, t, gp);
	/****************************************************************/
	...
	bool selfkill = false;
	if (check25(snake, len, t, gp, nextdirection) == false) { selfkill = true; temp = op; }//lose
	else
	{
	...
	if (temp != -1)
	{
		...
		else if (temp == 1)
		{
			...
			else if (selfkill == true) printf("粉因白自杀获胜。%d", machine_move25(snake, len, direct, t, gp));
		...
		{
			GOTO(snake[t][i].x, snake[t][i].y);                                                    //输出蛇身
			printf("%d", i);
		}
...
int main()
{
	...
	while (1)
	{
		...
		while (!_kbhit())
		{
			...
			t = !t;
			if (gp.speedowner == 0 || gp.speedowner == 1)                                                                           //两条蛇
				gp.speedowner = !t;
